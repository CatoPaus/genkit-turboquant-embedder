...
export default function Home() {
  const [activeUser, setActiveUser] = useState('my-test-user-123');
  ...
  useEffect(() => {
    // Clear UI state completely when switching users
    setMessages([]);
    setCostStats({ original: 0, compressed: 0, saved: 0 });

    // Bring back isolated history from Firestore Vector document logs
    fetch(`/api/chat/history?userId=${encodeURIComponent(activeUser)}`)
      .then(res => res.json())
      .then(data => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const result = streamFlow<typeof chatFlow>({
        url: '/api/chat',
        // Dynamic User ID enables completely isolated Firebase/Vertex Vector Memory namespaces immediately!
        input: { 
          userId: activeUser, 
          userMessage,
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-between p-24 bg-gradient-to-br from-indigo-900 via-purple-900 to-black text-white selection:bg-fuchsia-500 selection:text-white">
      ...
      <div className="z-10 max-w-5xl w-full flex-col items-center justify-center font-mono text-sm flex gap-8">
        ...
        {/* User Isolation Control */}
        <div className="flex justify-center items-center gap-3 bg-white/5 border border-white/10 rounded-full px-5 py-2 w-max mx-auto shadow-lg backdrop-blur-md">
          <span className="text-xs text-gray-400 uppercase tracking-widest font-semibold flex items-center gap-2">
            <svg className="w-4 h-4 text-cyan-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path></svg>
            Active Namespace:
          </span>
          <select 
            value={activeUser}
            onChange={(e) => setActiveUser(e.target.value)}
            className="bg-transparent text-fuchsia-400 font-bold text-sm outline-none cursor-pointer hover:text-fuchsia-300 transition-colors appearance-none"
          >
            <option value="my-test-user-123">Default Sandbox (my-test-user)</option>
            <option value="quantum-tester-001">Quantum Tester 001</option>
            <option value="quantum-tester-002">Quantum Tester 002</option>
            <option value="isolated-ghost-vector">Isolated Ghost Vector</option>
          </select>
        </div>
